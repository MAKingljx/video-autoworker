...
usage() {
  printf 'Usage: %s (--dry-run|--apply|--rollback) --target-sha <40-lowercase-hex-sha> [--backup <absolute-status-brief-upgrade-backup>]\n' "$0"
}
...
cleanup() {
  ...
  if [[ -n "$WORK_ROOT" && -d "$WORK_ROOT" ]]; then
    case "$WORK_ROOT" in
      /tmp/aiworker-video-status-brief-upgrade.*|/private/tmp/aiworker-video-status-brief-upgrade.*)
        rm -rf -- "$WORK_ROOT"
        ;;
...
WORK_ROOT="$(mktemp -d /tmp/aiworker-video-status-brief-upgrade.XXXXXX)"
chmod 700 "$WORK_ROOT"
trap cleanup EXIT
trap 'exit 129' HUP
trap 'exit 130' INT
...
const entries = readdirSync(root)
  .filter(name => /^status-brief-upgrade-[0-9]{8}-[0-9]{6}\.[A-Za-z0-9]+$/u.test(name))
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  printf 'Video status-brief upgrade dry-run passed for %s -> %s at %s.\n' "$PREVIOUS_VERSION" "$CANDIDATE_VERSION" "$TARGET_SHA"
...
if [[ "$MODE" == "rollback" ]]; then
  case "$ROLLBACK_BACKUP" in
    "$BACKUP_ROOT"/status-brief-upgrade-*) ;;
    *) printf 'Rollback backup is outside the approved status-brief family.\n' >&2; exit 1 ;;
  esac
...
BACKUP_DIR="$(mktemp -d "$BACKUP_ROOT/status-brief-upgrade-$(date +%Y%m%d-%H%M%S).XXXXXX")"
...
enforce_retention
printf 'Upgraded qwen-current %s -> %s with status-brief support at %s.\n' "$PREVIOUS_VERSION" "$CANDIDATE_VERSION" "$TARGET_SHA"
