...
父模式会先验证数据库所处的迁移阶段。只有 `049/050` 核心迁移存在、迁移最高版本精确为 `050`，
且 `051–057` 的迁移记录及对应后续表全部不存在时，才把尚未引入的 claim、lease、cleanup debt 和
director outbox 明确绑定为零；不会为通过检查创建这些表。已迁入现代阶段的数据库必须具备完整的
相关迁移记录、表和所需列，部分迁移、现代缺表或核心 schema 缺失均失败关闭。
迁移 ID、表存在性及表结构摘要纳入既有 prepare/apply 证据；准备后新建、删除或改动表和迁移记录
会使确认失效，必须重新分析与 prepare。该兼容仅适用于一次性的父记录对账，不改变运行时迁移或任务链。
