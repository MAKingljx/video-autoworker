...
当前生产仍以本节上方的旧视频任务链和当次远端验收为准。候选发布不会增加第二条任务链：用户可在
同一个 OpenClaw 单视频请求中提供作品名，3017 将其解析为可信、持久化的作品绑定；正式 finalize 在
提交既有成功终态的同一事务内追加唯一 evidence outbox。现有 scheduler 消费 outbox，经
`project-evidence` 写入作品级受控证据，再按“素材理解 -> 人物理解 -> 故事发现 -> 导演判断 -> 叙事结构”
五阶段生成待审核候选。飞书失败只重放同一 outbox 和稳定证据 ID，不回滚、重启或重提原视频任务。

这条扩展目前属于 `0.4.0` / schema v3 本地候选：测试飞书尚未迁移，3017 尚未切换，真实视频闭环尚未
验收。它只形成知识、判断和叙事候选，不控制 DaVinci、剪辑时间线、渲染、导出或任何媒体编辑副作用。
