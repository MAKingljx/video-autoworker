...
const repositoryIndex = rawArgs.indexOf('-C')
const repositoryRoot = repositoryIndex >= 0 ? rawArgs[repositoryIndex + 1] : process.cwd()
...
else if (args.includes('rev-parse --show-toplevel')) process.stdout.write(repositoryRoot + '\\n')
...
else if (args.includes('cat-file -e')) process.exit(0)
else if (args.includes('merge-base --is-ancestor')) process.exit(0)
else process.exit(2)
`)
