...
describe('task-chain tool upgrade script', () => {
  ...
  it('keeps the result-candidate refresh as an explicit 0.5.5 to 0.5.6 upgrade family', async () => {
    const script = await readFile(RESULT_SCRIPT_PATH, 'utf8')

    expect(script).toContain('PREVIOUS_VERSION="0.5.5"')
    expect(script).toContain('CANDIDATE_VERSION="0.5.6"')
