...
- 用户明确按视频标题、节目名、季集、文件名或其他自然语言关键词查询任务进度、状态、结果或入队情况：status_search；value 只逐字复制当前消息中最小且唯一的标题、文件名、季集号或关键词（例如 S03E03），不要复制整句，不得加入旧上下文、改写或补全。
- 用户明确要求完整、详细或全文学习报告，并携带完整任务编号：result_task；携带完整批次编号：result_batch。
- 用户明确要求完整、详细或全文学习报告，并按视频标题、节目名、季集、文件名或关键词查询：result_search；value 只逐字复制当前消息中最小且唯一的标题、文件名、季集号或关键词（例如 S03E03），不要复制整句，不得加入旧上下文、改写或补全。
