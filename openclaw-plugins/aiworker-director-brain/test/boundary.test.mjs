...
describe('director brain static boundaries', () => {
  ...
  it('keeps the skill explicit about candidate and non-editing behavior', async () => {
    ...
    expect(skill).toContain('不提供 `start_extraction` 或 `backfill_extraction`')
    expect(skill).toContain('`extraction_status` 只是 3017 loopback 共享应用服务的只读薄入口')
    expect(skill).toContain('不得从 OpenClaw 对话启动、回填、重提导演提炼任务或触发素材投影')
