...
function gitFile(repository, commit, relativePath, expectedPath, mode) {
  ...
  if (run('/usr/bin/git', ['-C', repository, 'rev-parse', '--verify', 'HEAD^{commit}'], 'source HEAD') !== commit
    || run('/usr/bin/git', ['-C', repository, 'status', '--porcelain=v1', '--untracked-files=all'], 'source status')) {
  ...
  const tree = run('/usr/bin/git', ['-C', repository, 'ls-tree', commit, '--', relativePath], 'source tree')
  let tracked
  try {
    tracked = execFileSync('/usr/bin/git', ['-C', repository, 'show', `${commit}:${relativePath}`], {
      encoding: null, timeout: 120_000, maxBuffer: 16 * 1024 * 1024,
    })
  } catch { fail('source file failed') }
  if (!tree.startsWith(`${treeMode} blob `) || sha256(tracked) !== sha256(loaded.source)) {
