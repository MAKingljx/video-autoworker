...
export function proxy(request: NextRequest) {
  ...
  const isLocalOpenClawProfilesNoAuth = isOpenClawProfilesNoAuthRequest(request, pathname)
  const isLocalDesktop = isLocalDesktopRequest(request)
  if (isLocalDesktop && (pathname === '/login' || pathname === '/setup')) {
  ...
  if (pathname === '/login' || pathname === '/setup' || pathname.startsWith('/api/auth/') || pathname === '/api/setup' || pathname === '/api/docs' || pathname === '/docs' || isPublicHealthProbe || isLocalOpenClawProfilesNoAuth) {
