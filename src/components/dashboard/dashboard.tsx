  ...
  const localSessionLogs: LogLike[] = isLocal
    ? sessions.reduce<LogLike[]>((acc, session) => {
        ...
        acc.push({
          ...
          message: lastPrompt
            ? `提示词：${lastPrompt}`
            : `${session.active ? '活跃' : '空闲'}会话：${session.key || session.id}`,
        })
        return acc
      }, [])
    : []
    ...
    <div className="p-5 space-y-4">
      ...
      <section className="rounded-xl border border-border bg-card p-4">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="text-2xs uppercase tracking-[0.12em] text-muted-foreground">概览</div>
            <h2 className="text-lg font-semibold text-foreground">
              {isLocal ? '本地智能体运行态' : 'OpenClaw 与本地千问控制台'}
            </h2>
            <p className="text-xs text-muted-foreground">
              {isLocal
                ? '统一查看 OpenClaw 网关、本地千问会话、主机压力和操作连续性。'
                : '集中管理 OpenClaw 网关、本地千问执行节点、会话路由和队列压力。'}
            </p>
          </div>
