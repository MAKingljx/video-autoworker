...
const navGroups: NavGroup[] = [
  {
    id: 'core',
    items: [
      { id: 'overview', label: '概览', icon: <OverviewIcon />, priority: true, essential: true },
      { id: 'agents', label: '智能体', icon: <AgentsIcon />, priority: true, essential: true },
      ...
      { id: 'memory', label: '记忆', icon: <MemoryIcon />, priority: false },
    ],
  },
  {
    id: 'observe',
    label: '监控',
    items: [
      { id: 'activity', label: '活动', icon: <ActivityIcon />, priority: true, essential: true },
      { id: 'logs', label: '日志', icon: <LogsIcon />, priority: false, essential: true },
      { id: 'cost-tracker', label: '费用追踪', icon: <TokensIcon />, priority: false },
      { id: 'nodes', label: '节点', icon: <NodesIcon />, priority: false },
      { id: 'exec-approvals', label: '审批', icon: <ApprovalsIcon />, priority: false },
      { id: 'office', label: '办公室', icon: <OfficeIcon />, priority: false },
      { id: 'monitor', label: '监控器', icon: <MonitorIcon />, priority: false },
    ],
  },
  {
    id: 'automate',
    ...
    items: [
      ...
      { id: 'alerts', label: '告警', icon: <AlertIcon />, priority: false },
  ...
  },
  {
    ...
    label: '管理',
    items: [
      { id: 'security', label: '安全', icon: <SecurityIcon />, priority: false },
      { id: 'users', label: '用户', icon: <UsersIcon />, priority: false },
      { id: 'audit', label: '审计', icon: <AuditIcon />, priority: false },
      {
        id: 'gateway-parent', label: '网关', icon: <GatewaysIcon />, priority: false,
        children: [
          { id: 'gateways', label: '网关', icon: <GatewaysIcon />, priority: false },
          { id: 'gateway-config', label: '配置', icon: <GatewayConfigIcon />, priority: false },
        ],
      },
      { id: 'integrations', label: '集成', icon: <IntegrationsIcon />, priority: false },
      { id: 'debug', label: '调试', icon: <DebugIcon />, priority: false },
      { id: 'settings', label: '设置', icon: <SettingsIcon />, priority: false, essential: true },
    ],
  },
]
...
function ContextSwitcher({ currentUser, isAdmin, isLocal, isConnected, tenants, osUsers, activeTenant, onSwitchTenant, projects, activeProject, onSwitchProject, expanded, defaultOrgName, navigateToPanel, fetchTenants, fetchOsUsers }: {
  ...
  const [createForm, setCreateForm] = useState({ username: '', display_name: '', gateway_port: '', install_openclaw: true })
              ...
              <>
                ...
                <div className="px-1">
                  ...
                  {unlinkedOsUsers.map((osUser) => {
                    const hasOpenClaw = Boolean(osUser.has_openclaw)
                    const disabled = isLocal && !hasOpenClaw
                    const tools = [hasOpenClaw && 'OpenClaw'].filter(Boolean)
                    ...
                    return (
                      <Button
                        ...
                        disabled={disabled}
                        title={disabled
                          ? `${osUser.username} 尚未安装 OpenClaw：${osUser.home_dir}`
                          : `${osUser.home_dir} (uid ${osUser.uid}) — 点击配置为组织`
                        }
                        className={`w-full flex items-center gap-2 px-2 py-1.5 h-auto rounded-md text-xs justify-start ${
                          disabled
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              setCreateForm({ username: '', display_name: '', gateway_port: '', install_openclaw: true })
                              setCreateMode(false)
...
function ProjectsIcon() {
  return (
    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M4.5 7.5h2.5M9 7.5h2.5M4.5 10.5h6.5" />
    </svg>
  )
}
...
function MaterialsIcon() {
  return (
    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M6.5 5l3 1.5-3 1.5V5z" />
      <path d="M3 13h10M5 10v3M11 10v3" />
    </svg>
  )
}
