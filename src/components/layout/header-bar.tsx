...
const QUICK_NAV_COMMANDS: Array<{ panel: string; titleKey: string; title: string; aliases: string[] }> = [
  { panel: 'overview', titleKey: 'goToOverview', title: '前往概览', aliases: ['home', 'dashboard'] },
  { panel: 'tasks', titleKey: 'goToTasks', title: '前往任务', aliases: ['task board', 'tickets'] },
  { panel: 'agents', titleKey: 'goToAgents', title: '前往智能体', aliases: ['agent squad', 'workers'] },
  { panel: 'activity', titleKey: 'goToActivityFeed', title: '前往活动流', aliases: ['events', 'feed'] },
  { panel: 'notifications', titleKey: 'goToNotifications', title: '前往通知', aliases: ['alerts inbox'] },
  { panel: 'tokens', titleKey: 'goToTokenUsage', title: '前往 Token 用量', aliases: ['cost', 'spend'] },
  { panel: 'logs', titleKey: 'goToLogs', title: '前往日志', aliases: ['log viewer'] },
  { panel: 'memory', titleKey: 'goToMemoryBrowser', title: '前往记忆浏览器', aliases: ['knowledge', 'notes'] },
  { panel: 'integrations', titleKey: 'goToIntegrations', title: '前往集成', aliases: ['providers', 'api keys'] },
  { panel: 'settings', titleKey: 'goToSettings', title: '前往设置', aliases: ['preferences', 'config'] },
  { panel: 'gateways', titleKey: 'goToGateways', title: '前往网关', aliases: ['gateway manager'] },
  { panel: 'github', titleKey: 'goToGithubSync', title: '前往 GitHub 同步', aliases: ['github', 'sync'] },
  { panel: 'office', titleKey: 'goToOffice', title: '前往办公室', aliases: ['workspace', 'team'] },
  { panel: 'skills', titleKey: 'goToSkills', title: '前往技能', aliases: ['skill packs', 'agent skills'] },
]
...
export function HeaderBar() {
  ...
  return (
    <header role="banner" aria-label="应用顶部栏" className="relative z-50 h-14 bg-card/80 backdrop-blur-sm border-b border-border px-3 md:px-4 shrink-0">
        ...
        <div className="flex min-w-0 items-center gap-2.5 shrink-0">
          {activeProject ? (
            <Button
              ...
              title={`当前项目：${activeProject.name}`}
            >
              <span className="text-muted-foreground/60 truncate">{activeTenant?.display_name || '默认'}</span>
      ...
      {searchOpen && isMounted && createPortal(
        <div
          ...
          aria-label="命令搜索"
        >
          <div className="absolute inset-0 bg-black/35" onClick={() => setSearchOpen(false)} />
...
function NavigationLatencyStat() {
  ...
  const avg = avgMs == null ? '' : `（平均 ${Math.round(avgMs)}ms）`
  return <Stat label="导航" value={`${latest}${avg}`} />
}
