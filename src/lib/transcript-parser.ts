...
import { getGatewayTranscriptPathAtStateDir } from '@/lib/openclaw-session-source'
...
export function readSessionJsonl(stateDir: string, agentName: string, sessionId: string): string | null {
  const jsonlPath = getGatewayTranscriptPathAtStateDir(stateDir, { agent: agentName, sessionId })
