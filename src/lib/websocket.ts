...
// Gateway protocol version (OpenClaw 2026.5+ requires v4).
...
export function useWebSocket() {
  ...
  const sendConnectHandshake = useCallback(async (ws: WebSocket, nonce?: string) => {
    ...
    const tokenForSignature = authToken ?? ''

    if (nonce && !tokenOnlyFallbackRef.current) {
      try {
        const identity = await getOrCreateDeviceIdentity()
        const signedAt = Date.now()
        // Sign the OpenClaw v3 device-auth payload used by protocol v4 gateways.
        const payload = [
          'v3',
          identity.deviceId,
          clientId,
    ...
    const deviceToken =
      !tokenOnlyFallbackRef.current && device && cachedToken
        ? cachedToken
        : undefined
    const auth =
  ...
  const handleGatewayFrame = useCallback((frame: GatewayFrame, ws: WebSocket) => {
    ...
    if (frame.type === 'res' && frame.ok && !handshakeCompleteRef.current) {
      ...
      const deviceToken = frame.result?.deviceToken || frame.payload?.auth?.deviceToken || frame.payload?.deviceToken
      if (deviceToken) {
