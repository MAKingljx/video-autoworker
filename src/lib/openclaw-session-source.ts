...
export function getGatewaySessionStoreFiles(): string[] {
...
export function getGatewaySessionByKey(sessionKey: string, force = false): GatewaySession | null {
  const normalizedKey = String(sessionKey || '').trim()
  if (!normalizedKey) return null
  return getAllGatewaySessions(Infinity, force).find((session) => session.key === normalizedKey) || null
}

export function getGatewayTranscriptPathAtStateDir(
  stateDir: string | null | undefined,
  session: Pick<GatewaySession, 'agent' | 'sessionId'>,
): string | null {
  const normalized = (stateDir || '').trim()
  if (!normalized || !session.agent || !session.sessionId) return null
  return path.join(normalized, 'agents', session.agent, 'sessions', `${session.sessionId}.jsonl`)
}

export function getGatewayTranscriptPath(session: Pick<GatewaySession, 'agent' | 'sessionId'>): string | null {
  return getGatewayTranscriptPathAtStateDir(config.openclawStateDir, session)
}
...
/**
 * Derive agent active/idle/offline status from OpenClaw-backed sessions.
 * Returns a map of agentName -> { status, lastActivity, channel }
 */
export function getOpenClawAgentLiveStatuses(): Map<string, {
...
export const getAgentLiveStatuses = getOpenClawAgentLiveStatuses
