...
function normalizeGatewayPath(pathname: string): string {
  ...
  if (
    ...
    return DEFAULT_GATEWAY_WS_PATH
  }
  return path === '/' ? DEFAULT_GATEWAY_WS_PATH : path.replace(/\/+$/, '')
}
...
export function buildGatewayWebSocketUrl(input: {
  ...
  if (!rawHost) {
    ...
    return `ws://127.0.0.1:${port || 18789}${DEFAULT_GATEWAY_WS_PATH}`
  }
  ...
  return shouldOmitPort
    ? `${wsProtocol}://${rawHost}${DEFAULT_GATEWAY_WS_PATH}`
    : `${wsProtocol}://${rawHost}:${port || 18789}${DEFAULT_GATEWAY_WS_PATH}`
}
