...
/** Extract the runtime agent identifier from the agent's config JSON.
 *  Prefers config.runtimeAgentId, then openclawId, then agent_name. */
function resolveGatewayAgentId(task: DispatchableTask): string {
  if (task.agent_config) {
    try {
      const cfg = JSON.parse(task.agent_config)
      if (typeof cfg.runtimeAgentId === 'string' && cfg.runtimeAgentId) return cfg.runtimeAgentId
