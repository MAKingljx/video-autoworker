...
import { resolveActiveGatewaySession } from '@/lib/sessions'
...
function extractReplyText(waitPayload: any): string | null {
  ...
  const directCandidates = [
    ...
    waitPayload.output,
    waitPayload.result,
    waitPayload.finalText,
    waitPayload.final_text,
    waitPayload.completion,
  ]
...
function appendBestEffortRuntimeReply(params: {
  db: ReturnType<typeof getDatabase>
  workspaceId: number
  conversationId: string
  runtimeSender: string
  recipient: string
  runId: string
}) {
  const { db, workspaceId, conversationId, runtimeSender, recipient, runId } = params
  const runtimeProvider = getRuntimeProvider()

  return runtimeProvider.waitForRun(runId, 6000).then((waitResult) => {
    const waitPayload = waitResult.raw
    ...
    if (waitStatus === 'timeout') {
      createChatReply(
        ...
        runtimeSender,
        recipient,
        'Message delivered. The runtime is still processing.',
        'status',
    ...
    createChatReply(
      ...
      runtimeSender,
      recipient,
      'Execution completed with no textual response payload.',
      'status',
      { status: waitStatus || 'completed', runId }
    )
  }).catch((waitErr) => {
    ...
    const reason =
      typeof waitPayload?.error === 'string'
        ? waitPayload.error
        : (maybeWaitStderr || maybeWaitStdout || 'Unable to read completion status from runtime.').trim()
    ...
    createChatReply(
      ...
      runtimeSender,
      recipient,
      `Message delivered, but completion output is not available yet: ${reason}`,
      'status',
    ...
    if (to) {
      ...
      if (body.forward) {
        ...
        const explicitSessionKey = typeof body.sessionKey === 'string' && body.sessionKey
          ...
          : null
        const runtimeProvider = getRuntimeProvider()
        const sessions = await runtimeProvider.listSessions()
        ...
        const matchedSession = resolveActiveGatewaySession(
          sessions,
          [
            String(to),
            coordinatorResolution.deliveryName,
            coordinatorResolution.runtimeAgentId,
            coordinatorResolution.openclawAgentId,
          ],
          {
            preferredSessionKey: coordinatorResolution.sessionKey,
          },
        )

        const sessionKey: string | null = matchedSession?.key || matchedSession?.sessionId || coordinatorResolution.sessionKey || null

        // Prefer configured openclawId when present, fallback to normalized name
        let openclawAgentId: string | null = coordinatorResolution.runtimeAgentId || coordinatorResolution.openclawAgentId
        ...
        if (!sessionKey && !openclawAgentId) {
          ...
          try {
            ...
            const sendResult = await runtimeProvider.sendMessage({
              sessionKey,
              agentId: openclawAgentId,
              message: sessionKey ? content : `Message from ${from}: ${content}`,
              idempotencyKey,
              attachments: sessionKey ? toGatewayAttachments(body.attachments) : undefined,
              deliver: false,
            })
            ...
            forwardInfo.delivered =
              ...
              status === 'ok' ||
              status === 'in_flight'
            forwardInfo.session = sendResult.session || sessionKey || openclawAgentId || undefined
            if (sendResult.runId) {
              forwardInfo.runId = sendResult.runId
            }
          } catch (err) {
            ...
            if (maybeStdout.includes('"status": "accepted"') || maybeStdout.includes('"status":"accepted"')) {
              ...
              logger.error({ err }, 'Failed to forward message via runtime provider')
          ...
          if (
            ...
            if (forwardInfo.runId) {
              await appendBestEffortRuntimeReply({
                db,
                workspaceId,
                conversationId: conversation_id,
                runtimeSender: COORDINATOR_AGENT,
                recipient: from,
                runId: forwardInfo.runId,
              })
            }
          } else if (forwardInfo.delivered && forwardInfo.runId && typeof conversation_id === 'string') {
            await appendBestEffortRuntimeReply({
              db,
              workspaceId,
              conversationId: conversation_id,
              runtimeSender: String(to),
              recipient: from,
              runId: forwardInfo.runId,
            })
          }
