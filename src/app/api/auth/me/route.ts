...
export async function GET(request: Request) {
  ...
  if (localDesktopUser) {
    return NextResponse.json({ user: localDesktopUser })
  }
...
function getLocalProfilesDesktopUser(request: Request) {
  if (!envFlag('MC_OPENCLAW_PROFILES_NO_AUTH') && !envFlag('MC_DESKTOP_MODE')) return null
  if (!isLoopbackHost(new URL(request.url).host)) return null
