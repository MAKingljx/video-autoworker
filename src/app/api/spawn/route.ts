...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // Call provider spawn. OpenClaw provider preserves existing compatibility
      // fallback behavior for older gateways that don't support tools.profile.
      const result: any = await runtimeProvider.spawnSession(spawnPayload)
      const compatibilityFallbackUsed = Boolean(result?.__compatibilityFallbackUsed)
