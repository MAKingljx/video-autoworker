...
import { readGitProductFile, resolveGitSourceLayout } from './lib/git-source-layout.mjs'
...
function gitSource(repository, expectedCommit, pathname) {
  if (process.env.NODE_ENV === 'test' && process.env.AIWORKER_TEST_N8N_IDENTITY === '1'
    ...
    try {
      const layout = resolveGitSourceLayout(repository)
      return readGitProductFile(layout.gitRoot, expectedCommit, pathname).toString('utf8')
    } catch {
  ...
  try {
    const layout = resolveGitSourceLayout(repository)
    return readGitProductFile(layout.gitRoot, expectedCommit, pathname).toString('utf8')
  } catch { fail(`Git source read for ${pathname}`) }
}
