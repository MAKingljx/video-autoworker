...
function gitCommitReader(repositoryRoot, commit) {
  ...
  const productPrefix = resolveGitCommitProductPrefix(gitRoot, commit)
  const tree = execFileSync('git', [
    '-C', gitRoot, 'ls-tree', '-r', '-z', commit,
  ...
  for (const record of tree.toString('utf8').split('\0')) {
    ...
    if (!match) continue
    const treePath = match[2]
    if (productPrefix && !treePath.startsWith(NESTED_PRODUCT_PREFIX)) continue
    const productPath = productPrefix ? treePath.slice(NESTED_PRODUCT_PREFIX.length) : treePath
    if (productPath) modes.set(productPath, match[1])
  }
  return {
    has(sourcePath) {
    ...
    read(sourcePath) {
      ...
      return execFileSync('git', [
        '-C', gitRoot, 'show', `${commit}:${productTreePath(productPrefix, normalized)}`,
      ], {
        encoding: 'buffer',
        stdio: ['ignore', 'pipe', 'ignore'],
        maxBuffer: 64 * 1024 * 1024,
...
export function createStandaloneBuildSourceAnchor(
  ...
  const { gitRoot } = resolveGitSourceLayout(repositoryRoot)
  const gitCommit = gitOutput(gitRoot, ['rev-parse', '--verify', 'HEAD^{commit}'])
  const gitStatus = gitOutput(gitRoot, ['status', '--porcelain=v1', '--untracked-files=all'])
