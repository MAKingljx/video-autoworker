...
const GET_MANY_READ_CONCURRENCY = 4

async function mapReadOnlyBatch(values, operation) {
  const results = new Array(values.length)
  let cursor = 0
  let failed = false
  let firstError
  const workers = Array.from(
    { length: Math.min(GET_MANY_READ_CONCURRENCY, values.length) },
    async () => {
      while (!failed) {
        const index = cursor
  ...
  // Workers capture errors instead of rejecting, so Promise.all waits for every
  // already-active read before the first failure is returned to the caller.
...
export async function executeDirectorBrainOperation(requestValue, options = {}) {
  ...
  if (request.action === 'get_many') {
    ...
    const { tableId } = tableContext(context.schema, context.catalog, table.key)
    const outcomes = await mapReadOnlyBatch(stableIds, async stableId => {
      ...
      if (found.length === 0) return { stableId, record: null }
    ...
    for (const outcome of outcomes) {
      ...
      else missing.push(outcome.stableId)
    }
    return {
      ok: true,
      action: 'get_many',
