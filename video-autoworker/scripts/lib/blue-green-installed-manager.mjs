...
function verifyManagerGitBinding(sourceRoot, expectedCommit) {
  ...
  let layout
  try { layout = assertCleanGitSource(sourceRoot, expectedCommit) } catch {
    fail('manager source worktree is not the exact clean Git source')
  }
  ...
  let committed
  try {
    entry = getGitProductFileEntry(layout.gitRoot, expectedCommit, MANAGER_RELATIVE_PATH)
    committed = readGitProductFile(layout.gitRoot, expectedCommit, MANAGER_RELATIVE_PATH)
  } catch { fail('installed blue-green manager Git binding is unavailable') }
