...
export async function assertStandaloneStaticImportClosure(
  ...
  for (const importer of [
    ...
    const importerStat = await lstat(importerPath).catch(() => null)
    if (!importerStat?.isFile() || importerStat.isSymbolicLink()) continue
    const source = await readFile(importerPath, 'utf8')
    if (!source.includes('scripts/lib/git-source-layout.sh')) continue
    dynamicRuntimeDependencies.set(importer, [
      ...(dynamicRuntimeDependencies.get(importer) || []),
      'scripts/lib/git-source-layout.sh',
    ])
  }
