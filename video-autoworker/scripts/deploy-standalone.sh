...
[[ -f "$GIT_SOURCE_LAYOUT_SHELL" && ! -L "$GIT_SOURCE_LAYOUT_SHELL" ]] \
  || { printf 'Git source layout bootstrap is unavailable.\n' >&2; exit 1; }
LAYOUT_NODE_BIN="$(node -e 'process.stdout.write(process.execPath)')" \
  || { printf 'Unable to resolve the Node.js executable.\n' >&2; exit 1; }
# shellcheck source=scripts/lib/git-source-layout.sh
source "$GIT_SOURCE_LAYOUT_SHELL"
assert_git_source_layout_helper_bootstrap "$PROJECT_ROOT" "$LAYOUT_NODE_BIN" \
  || { printf 'Git source layout helper is not bound to the current Git HEAD.\n' >&2; exit 1; }
"${NODE_BIN:-node}" "$GIT_SOURCE_LAYOUT" assert-clean "$PROJECT_ROOT" >/dev/null \
  || { printf 'Deployment source must be the exact clean canonical Git source.\n' >&2; exit 1; }
