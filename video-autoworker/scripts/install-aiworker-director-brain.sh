...
GIT_SOURCE_LAYOUT_SHELL="$REPOSITORY_ROOT/scripts/lib/git-source-layout.sh"
...
assert_canonical_source_repository() {
  local expected_commit="${1:-}" layout current_commit source_path relative
  local -a layout_arguments=(assert-clean "$REPOSITORY_ROOT")
  [[ -f "$GIT_SOURCE_LAYOUT" && ! -L "$GIT_SOURCE_LAYOUT" \
    && -f "$GIT_SOURCE_LAYOUT_SHELL" && ! -L "$GIT_SOURCE_LAYOUT_SHELL" ]] || return 1
  # shellcheck source=scripts/lib/git-source-layout.sh
  source "$GIT_SOURCE_LAYOUT_SHELL"
  assert_git_source_layout_helper_bootstrap "$REPOSITORY_ROOT" "$NODE_BIN" || return 1
  [[ -z "$expected_commit" ]] || layout_arguments+=("$expected_commit")
  layout="$("$NODE_BIN" "$GIT_SOURCE_LAYOUT" "${layout_arguments[@]}")" || return 1
  current_commit="$(printf '%s' "$layout" | "$NODE_BIN" -e '
    const value = JSON.parse(require("node:fs").readFileSync(0, "utf8"))
    if (value.productRoot !== process.argv[1] || !/^[a-f0-9]{40}$/u.test(value.headCommit)) process.exit(2)
    process.stdout.write(value.headCommit)
  ' "$REPOSITORY_ROOT")" || return 1
  while IFS= read -r source_path; do
    relative="${source_path#"$REPOSITORY_ROOT"/}"
    [[ "$relative" != "$source_path" ]] \
      && "$NODE_BIN" "$GIT_SOURCE_LAYOUT" verify-file \
        "$REPOSITORY_ROOT" "$relative" "$current_commit" >/dev/null 2>&1 || {
...
verify_shared_install_gate() {
  ...
  if [[ "$MUTATION_AUTHORIZATION" == production ]]; then
    ...
    STABLE_QUEUE_SHA256="$(printf '%s' "$gate_output" | node -e '
      const value = JSON.parse(require("node:fs").readFileSync(0, "utf8"))
    ...
    ' "$STABLE_QUEUE_SHA256")" || {
      printf 'Held maintenance queue changed during director-brain installation.\n' >&2
      return 1
    }
