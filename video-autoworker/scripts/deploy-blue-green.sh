...
GIT_SOURCE_LAYOUT="$PROJECT_ROOT/scripts/lib/git-source-layout.mjs"
GIT_SOURCE_LAYOUT_SHELL="$PROJECT_ROOT/scripts/lib/git-source-layout.sh"
NODE_BIN="${NODE_BIN:-node}"
NODE_BIN="$("$NODE_BIN" -e 'process.stdout.write(process.execPath)')" || {
  printf 'error: unable to resolve the Node.js executable\n' >&2
  exit 1
}
[[ "$NODE_BIN" == /* && -f "$NODE_BIN" && ! -L "$NODE_BIN" && -x "$NODE_BIN" ]] || {
  printf 'error: Node.js executable is unsafe\n' >&2
  exit 1
}
...
verify_deployment_source_gate() {
  ...
  local -a critical_paths=(
    ...
    scripts/lib/git-source-layout.sh
  ...
  [[ -f "$GIT_SOURCE_LAYOUT" && ! -L "$GIT_SOURCE_LAYOUT" ]] \
    || fail "Git source layout helper is unavailable"
  [[ -f "$GIT_SOURCE_LAYOUT_SHELL" && ! -L "$GIT_SOURCE_LAYOUT_SHELL" ]] \
    || fail "Git source layout bootstrap is unavailable"
  # shellcheck source=scripts/lib/git-source-layout.sh
  source "$GIT_SOURCE_LAYOUT_SHELL"
  assert_git_source_layout_helper_bootstrap "$PROJECT_ROOT" "$NODE_BIN" \
    || fail "Git source layout helper is not bound to the current Git HEAD"
  layout="$("$NODE_BIN" "$GIT_SOURCE_LAYOUT" resolve "$PROJECT_ROOT")" \
    || fail "deployment source is not inside the canonical Git layout"
  ...
  "$NODE_BIN" -e '
    ...
    if (value.productRoot !== process.argv[2]
...
check_n8n_workflow_compatibility() {
  ...
  "$NODE_BIN" "$GIT_SOURCE_LAYOUT" assert-clean "$verifier_root" "$expected_commit" \
    >/dev/null || fail "n8n workflow compatibility source is not the exact clean Git source"
  for relative in \
    ops/n8n/workflows/aiworker-task-intake.json \
    ops/n8n/workflows/aiworker-video-analysis.json; do
    "$NODE_BIN" "$GIT_SOURCE_LAYOUT" verify-file \
      "$verifier_root" "$relative" "$expected_commit" >/dev/null \
...
NODE
  ...
  "$NODE_BIN" "$GIT_SOURCE_LAYOUT" assert-clean \
    "$BOOTSTRAP_HISTORICAL_PROJECT_ROOT" "$authorization_source_commit" >/dev/null \
    || fail "managed historical source is not the exact clean Git source"
  "$NODE_BIN" "$GIT_SOURCE_LAYOUT" verify-file "$BOOTSTRAP_HISTORICAL_PROJECT_ROOT" \
    scripts/generate-legacy-freeze-evidence.mjs "$authorization_source_commit" >/dev/null \
  ...
  "$NODE_BIN" "$GIT_SOURCE_LAYOUT" verify-file "$BOOTSTRAP_HISTORICAL_PROJECT_ROOT" \
    scripts/generate-legacy-bootstrap-rollback-proof.mjs "$authorization_source_commit" >/dev/null \
  ...
  "$NODE_BIN" "$GIT_SOURCE_LAYOUT" verify-file "$BOOTSTRAP_HISTORICAL_PROJECT_ROOT" \
    scripts/legacy-freeze-guard.mjs "$authorization_source_commit" >/dev/null \
  ...
  "$NODE_BIN" "$GIT_SOURCE_LAYOUT" verify-file "$BOOTSTRAP_HISTORICAL_PROJECT_ROOT" \
    scripts/legacy-bootstrap-controller.mjs "$authorization_source_commit" >/dev/null \
