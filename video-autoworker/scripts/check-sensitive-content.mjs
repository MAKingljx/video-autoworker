...
function loadAllowlist(content, productPrefix = '') {
  let value
  try {
  ...
  for (const entry of value.entries) {
    const productPath = normalizedRepositoryPath(entry?.path)
    const path = productTreePath(productPrefix, productPath)
    const type = entry?.type
    const fingerprint = entry?.fingerprint
    const occurrences = entry?.occurrences
    const reason = entry?.reason
    if (!isTestFixturePath(productPath) || typeof type !== 'string' || !type
...
export function scanSourceSensitiveContent(options = {}) {
  const productRoot = physicalDirectory(options.repositoryRoot || MODULE_ROOT, 'sensitive_source_root')
  const layout = resolveGitSourceLayout(productRoot)
  const revision = options.commit || null
  const commit = revision ? resolveGitCommit(layout.gitRoot, revision) : null
  const productPrefix = commit
    ? resolveGitCommitProductPrefix(layout.gitRoot, commit)
    : layout.productPrefix
  const members = commit
    ? gitCommitMembers(layout.gitRoot, commit)
    : worktreeMembers(layout.gitRoot)
  const readMember = member => commit
    ? gitCommitFile(layout.gitRoot, commit, member)
    : safeWorktreeFile(layout.gitRoot, member)
  const findings = []
  let bytesScanned = 0
  for (const member of members) {
    const content = readMember(member)
    bytesScanned += content.length
    const logicalPath = productPrefix && member.startsWith(productPrefix)
      ? member.slice(productPrefix.length)
      : member
    findings.push(...scanReleaseFile(logicalPath, content, {
      contextual: !isDependencyLockfile(logicalPath),
      allowTestPlaceholders: true,
    }).map(finding => ({ ...finding, path: member })))
  }
  ...
  const allowlist = allowlistPath
    ? loadAllowlist(readMember(productTreePath(productPrefix, allowlistPath)), productPrefix)
    : new Map()
