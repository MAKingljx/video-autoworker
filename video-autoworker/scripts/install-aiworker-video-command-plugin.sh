...
SUPPORTED_PREVIOUS_VERSIONS=("0.5.8" "0.5.9" "0.5.10" "0.5.11" "0.5.12" "0.5.13" "0.5.14" "0.5.15" "0.5.16" "0.5.17" "0.5.18")
...
verify_shared_install_gate() {
  ...
  if [[ "$MUTATION_AUTHORIZATION" == production ]]; then
    ...
    ' "$STABLE_QUEUE_SHA256")" || {
      printf 'Held maintenance queue changed during video-command installation.\n' >&2
      return 1
    }
