...
function gitFile(repository, commit, relativePath, expectedPath, expectedMode) {
  ...
  let layout
  try { layout = assertCleanGitSource(repository, commit) } catch {
    fail('Git source is not the exact clean repository')
  }
  ...
  const gitMode = expectedMode === 0o755 ? '100755' : '100644'
  if (entry.mode !== gitMode) fail(`${relativePath} Git mode changed`)
