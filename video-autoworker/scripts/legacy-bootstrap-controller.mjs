...
function assertManagedRuntimeMember(pathname, relativePath, label) {
  ...
  let committed
  try {
    const layout = resolveGitSourceLayout(repositoryRoot)
    entry = getGitProductFileEntry(layout.gitRoot, 'HEAD', relativePath)
    committed = readGitProductFile(layout.gitRoot, 'HEAD', relativePath)
  } catch {
