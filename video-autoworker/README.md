...
Source versions are declared in the plugin packages and runtime convergence manifest. Production
versions are recorded only after installation, switching, and live acceptance; see the dated
operations records for the current deployment. DaVinci integration, editing-software timelines,
rendering, export, and other editing side effects remain outside the current scope.
...
Use the Node.js version in `.nvmrc` and the pnpm version declared by `package.json`.

## Local Start

The Git repository is managed from the AI-worker workspace root. Keep this product directory in
place and run the commands below from `AI-worker/video-autoworker/`. See the [workspace guide](../README.md).
