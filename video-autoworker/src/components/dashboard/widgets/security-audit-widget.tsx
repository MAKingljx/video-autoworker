...
const postureBadge: Record<string, { label: string; className: string }> = {
  hardened: { label: '已加固', className: 'bg-green-500/15 text-green-400' },
  secure: { label: '安全', className: 'bg-green-500/10 text-green-300' },
  'needs-attention': { label: '需要关注', className: 'bg-yellow-500/15 text-yellow-400' },
  'at-risk': { label: '存在风险', className: 'bg-red-500/15 text-red-400' },
}
  ...
  return (
    <div className="panel">
      <div className="panel-header">
        <h3 className="text-sm font-semibold">安全与审计</h3>
        {posture && badge && (
      ...
      <div className="panel-body space-y-3">
        <StatRow label="审计事件（24 小时）" value={dbStats?.audit.day ?? 0} />
        <StatRow label="审计事件（7 天）" value={dbStats?.audit.week ?? 0} />
        <StatRow label="登录失败（24 小时）" value={dbStats?.audit.loginFailures ?? 0} alert={dbStats ? dbStats.audit.loginFailures > 0 : false} />
        <StatRow label="未读通知" value={dbStats?.notifications.unread ?? 0} alert={(dbStats?.notifications.unread ?? 0) > 0} />
        <button
