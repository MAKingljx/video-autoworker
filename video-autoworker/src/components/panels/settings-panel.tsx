...
import type { GatewaySession } from '@/lib/openclaw-session-source'
...
const HIDDEN_SETTING_KEYS = new Set(['subscription.codex_plan'])
...
export function SettingsPanel() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      const visibleSettings = (data.settings || []).filter((setting: Setting) => !HIDDEN_SETTING_KEYS.has(setting.key))
      const visibleGrouped = Object.fromEntries(
        Object.entries(data.grouped || {}).map(([category, settings]) => [
          category,
          (settings as Setting[]).filter((setting) => !HIDDEN_SETTING_KEYS.has(setting.key)),
        ]),
      )
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      ...
      {currentUser?.role === 'admin' && (
        <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 text-xs text-muted-foreground">
          <strong className="text-foreground">{t('workspaceManagementLabel')}</strong>{' '}
          ...
          <div className="bg-card border border-border rounded-lg p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  用于程序化访问，以及通过 X-Api-Key 请求头或 Bearer Token 进行智能体认证。
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 flex items-center gap-2">
              <code className="text-xs font-mono bg-background border border-border rounded px-2 py-1 text-muted-foreground">
                {apiKeyLoading ? '加载中...' : apiKeyInfo?.masked_key || '未配置 API Key'}
              </code>
            </div>

            {apiKeyInfo?.last_rotated_at && (
              <div className="text-2xs text-muted-foreground/50 mt-2">
                上次轮换人：{apiKeyInfo.last_rotated_by}，日期：{' '}
                {new Date(apiKeyInfo.last_rotated_at * 1000).toLocaleDateString()}{' '}
                {new Date(apiKeyInfo.last_rotated_at * 1000).toLocaleTimeString()}
              </div>
            )}
