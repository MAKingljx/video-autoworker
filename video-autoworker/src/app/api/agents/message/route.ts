...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // NOTE: "session_key" in DB is not guaranteed to match the *live* runtime session.
    // Align delivery with the working coordinator/task dispatch pipelines by targeting
    // the active gateway session store key (e.g. "agent:main:main").

    const runtimeProvider = getRuntimeProvider()
    const sessions = await runtimeProvider.listSessions()

    // 1) Prefer an active runtime session for the agent by name.
    const matchedSession = resolveActiveGatewaySession(
      sessions,
      [String(to)],
      {
        fallbackSessionKey: typeof agent.session_key === 'string' ? agent.session_key : null,
        fallbackToMain: true,
      },
    )

    const sessionKey: string | null = matchedSession?.key || matchedSession?.sessionId || null

    if (!sessionKey) {
      return NextResponse.json(
        { error: 'No active gateway session found for recipient agent' },
        { status: 409 }
      )
    }

    await runtimeProvider.sendMessage({
      sessionKey,
      message: `Message from ${from}: ${message}`,
      idempotencyKey: `direct-message-${Date.now()}`,
      deliver: false,
    })
