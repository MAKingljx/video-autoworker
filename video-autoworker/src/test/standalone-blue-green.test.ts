...
`)
  ...
  it('keeps switch and rollback transport-only and pins n8n callbacks to the backend slot', () => {
    ...
    expect(retireBody.indexOf('scheduler_summary="$(check_json_endpoint scheduler'))
      .toBeLessThan(retireBody.indexOf('drain_retiring_router_connections "$slot"'))
    expect(retireBody.indexOf('drain_retiring_router_connections "$slot"'))
      .toBeLessThan(retireBody.indexOf('write_json_atomic "$(callback_freeze_file "$slot")"'))
    expect(retireBody).toContain('still owns non-reconnectable router requests')
