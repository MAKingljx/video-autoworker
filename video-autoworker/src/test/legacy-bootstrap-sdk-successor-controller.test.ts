...
function writeProductMarkers(repository: string) {
...
function fixture(options: { historicalControllerMode?: number } = {}) {
  ...
  const gitSourceLayoutPath = join(control, 'scripts/lib/git-source-layout.mjs')
  copyFileSync(resolve(process.cwd(), 'scripts/lib/git-source-layout.mjs'), gitSourceLayoutPath)
  chmodSync(gitSourceLayoutPath, 0o644)
...
`, { mode: options.historicalControllerMode ?? 0o755 })
  ...
  writeProductMarkers(historical)
  writeProductMarkers(control)
  ...
  const compatibilityCore = {
    ...
    plugins: { video: { id: 'aiworker-video-command', version: '0.5.16', peerPolicy: '>=2026.9.2' }, director: { id: 'aiworker-director-brain', version: '0.4.4', peerPolicy: '>=2026.9.2' } },
...
describe('legacy bootstrap SDK successor controller', () => {
  it('matches every controller-bound fixture file to its real Git tree mode', () => {
    ...
    const expected = [
      ...
      [entry.control, entry.controlCommit, 'scripts/lib/git-source-layout.mjs', '100644'],
    ] as const
