...
describe('Feishu director brain OpenClaw operation service', () => {
  ...
  it('bounds get_many read concurrency while preserving input order and missing IDs', async () => {
    ...
    const schema = await directorBrain.loadDirectorBrainSchema()
    const harness = operationHarness(schema, reviewedFoundation())
    const stableIds = Array.from({ length: 8 }, (_, index) => (
      `EVIDENCE-BATCH-${String(index + 1).padStart(2, '0')}`
    ))
    const base = harness.records.get('material_evidence')?.[0]
    let active = 0
    let maximumActive = 0
    const completionOrder: string[] = []
    ;(harness.options.dependencies as any).findExact = async ({ stableId }: {
      stableId: string
    }) => {
      active += 1
      maximumActive = Math.max(maximumActive, active)
      const index = stableIds.indexOf(stableId)
      await new Promise(resolveDelay => setTimeout(resolveDelay, (stableIds.length - index) * 2))
      active -= 1
      completionOrder.push(stableId)
      if (stableId === stableIds[3]) return []
      return [{
        record_id: `rec_${stableId}`,
        fields: { ...structuredClone(base?.fields || {}), '证据 ID': stableId },
      }]
    }

    const result = await directorBrain.executeDirectorBrainOperation({
      action: 'get_many', table: 'material_evidence', workId: 'WORK-ICE-001', stableIds,
    }, harness.options)

    expect(maximumActive).toBe(4)
    expect(completionOrder).not.toEqual(stableIds)
    expect(result).toMatchObject({ count: 7, missing: [stableIds[3]] })
    expect((result.records as Array<Record<string, unknown>>).map(record => record.stableId))
      .toEqual(stableIds.filter(stableId => stableId !== stableIds[3]))
    expect(harness.createCalls).toHaveLength(0)
    expect(harness.updateCalls).toHaveLength(0)
    const readsBeforeDuplicate = completionOrder.length
    await expect(directorBrain.executeDirectorBrainOperation({
      action: 'get_many', table: 'material_evidence', workId: 'WORK-ICE-001',
      stableIds: [stableIds[0], stableIds[0]],
    }, harness.options)).rejects.toThrow('operation_batch_duplicate_identity:stableIds')
    expect(completionOrder).toHaveLength(readsBeforeDuplicate)
  })

  it('stops scheduling get_many after the first failure and awaits active reads', async () => {
    const directorBrain = await loadModule()
    const schema = await directorBrain.loadDirectorBrainSchema()
    const harness = operationHarness(schema, reviewedFoundation())
    const stableIds = Array.from({ length: 8 }, (_, index) => `EVIDENCE-FAIL-${index + 1}`)
    const base = harness.records.get('material_evidence')?.[0]
    ...
    ;(harness.options.dependencies as any).findExact = async ({ stableId }: {
      ...
      started.push(stableId)
      active += 1
      maximumActive = Math.max(maximumActive, active)
      try {
        await new Promise(resolveDelay => setTimeout(
          resolveDelay,
          stableId === stableIds[0] ? 1 : 20,
        ))
        if (stableId === stableIds[0]) return [{
          record_id: `rec_${stableId}`,
          fields: {
            ...structuredClone(base?.fields || {}),
            '证据 ID': stableId,
            '作品 ID': 'WORK-OTHER',
          },
        }]
    ...
    await expect(directorBrain.executeDirectorBrainOperation({
      action: 'get_many', table: 'material_evidence', workId: 'WORK-ICE-001', stableIds,
    }, harness.options)).rejects.toThrow('record_work_mismatch:material_evidence')
    expect(started).toEqual(stableIds.slice(0, 4))
    expect(maximumActive).toBe(4)
