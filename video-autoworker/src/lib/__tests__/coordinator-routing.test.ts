...
describe('resolveCoordinatorDeliveryTarget', () => {
  ...
  it('prefers runtimeAgentId when provided for generic runtimes', () => {
    const directAgent: CoordinatorAgentRecord = {
      name: 'dev',
      config: JSON.stringify({ runtimeAgentId: 'agent://dev', openclawId: 'dev' }),
    }
  ...
  it('falls back to normalized destination when nothing else matches', () => {
    ...
    expect(resolved).toEqual({
      ...
      runtimeAgentId: 'coordinator-team',
      resolvedBy: 'fallback',
    })
  })
})
