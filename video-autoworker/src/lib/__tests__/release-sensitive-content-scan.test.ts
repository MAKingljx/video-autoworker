...
function writeProductMarkers(root: string): void {
  write(root, 'package.json', '{"name":"video-autoworker"}\n')
...
describe('release sensitive-content scanner', () => {
  ...
  it('allows only exact, counted test fingerprints and rejects stale allowances', () => {
    ...
    git(root, 'init', '-b', 'main')
    git(root, 'config', 'user.name', 'Sensitive Allowlist Test')
    git(root, 'config', 'user.email', 'sensitive-allowlist@example.invalid')
    git(root, 'config', 'commit.gpgSign', 'false')
    writeProductMarkers(root)
    ...
    git(root, 'add', '.')
    git(root, 'commit', '-m', 'allowlist fixture')
  ...
  it('prefixes product allowlists while still scanning sensitive files at the repository root', () => {
    const gitRoot = realpathSync(mkdtempSync(join(tmpdir(), 'sensitive-source-prefixed-')))
    const productRoot = join(gitRoot, 'video-autoworker')
    try {
      git(gitRoot, 'init', '-b', 'main')
      git(gitRoot, 'config', 'user.name', 'Prefixed Sensitive Scan Test')
      git(gitRoot, 'config', 'user.email', 'prefixed-sensitive@example.invalid')
      git(gitRoot, 'config', 'commit.gpgSign', 'false')
      writeProductMarkers(productRoot)
      const token = ['ghp', '_', 'Y'.repeat(36)].join('')
      write(productRoot, 'tests/detector.test.ts', `export const fixture = ${JSON.stringify(token)}\n`)
      write(productRoot, 'scripts/allowlist.json', `${JSON.stringify({
        schema: 'video-autoworker-sensitive-content-source-allowlist/v1',
        entries: [{
          path: 'tests/detector.test.ts',
          type: 'github_token',
      ...
      write(gitRoot, 'README.md', 'repository root\n')
      git(gitRoot, 'add', '.')
      git(gitRoot, 'commit', '-m', 'prefixed product')
      ...
      write(gitRoot, 'public-config.js', "export const apiKey = 'root-hardcoded-secret'\n")
      try {
        scanSourceSensitiveContent({
          repositoryRoot: productRoot,
          allowlistPath: 'scripts/allowlist.json',
        })
        throw new Error('expected root sensitive file to fail')
      } catch (error) {
        expect((error as Error & { report?: unknown }).report).toMatchObject({
          findings: [{ path: 'public-config.js', type: 'hardcoded_credential' }],
        })
      }
    } finally {
  ...
  it('uses product-relative message semantics without skipping high-confidence secrets', () => {
    const gitRoot = realpathSync(mkdtempSync(join(tmpdir(), 'sensitive-source-messages-')))
    const productRoot = join(gitRoot, 'video-autoworker')
    try {
      git(gitRoot, 'init', '-b', 'main')
      git(gitRoot, 'config', 'user.name', 'Prefixed Messages Test')
      git(gitRoot, 'config', 'user.email', 'prefixed-messages@example.invalid')
      git(gitRoot, 'config', 'commit.gpgSign', 'false')
      writeProductMarkers(productRoot)
      write(productRoot, 'messages/zh.json', JSON.stringify({
        settings: Object.fromEntries([
          ['password', '密码'], ['token', '访问令牌'], ['apiKey', '接口密钥'],
        ]),
      }))
      write(
        productRoot,
        'scripts/take-screenshots.ts',
        "export const apiKey = 'must-not-persist-fixture'\n",
      )
      git(gitRoot, 'add', '.')
      git(gitRoot, 'commit', '-m', 'safe prefixed translations')

      expect(scanSourceSensitiveContent({ repositoryRoot: productRoot })).toMatchObject({
        ok: true,
      })

      const secret = ['ghp', '_', 'Q'.repeat(36)].join('')
      write(productRoot, 'messages/zh.json', JSON.stringify({ notice: secret }))
      try {
        scanSourceSensitiveContent({ repositoryRoot: productRoot })
        throw new Error('expected prefixed message secret to fail')
      } catch (error) {
        expect((error as Error & { report?: unknown }).report).toMatchObject({
          findings: [{
            path: 'video-autoworker/messages/zh.json',
            type: 'github_token',
          }],
        })
      }
