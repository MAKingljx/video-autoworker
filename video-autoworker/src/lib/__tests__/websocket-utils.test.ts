...
describe('shouldRetryWithoutDeviceIdentity', () => {
  it('retries once for DEVICE_AUTH_SIGNATURE_INVALID when an auth token exists', () => {
    expect(
      shouldRetryWithoutDeviceIdentity(
  ...
  it('retries once for AUTH_DEVICE_TOKEN_MISMATCH when an auth token exists', () => {
    expect(
      shouldRetryWithoutDeviceIdentity(
        'cached device token mismatch',
...
describe('ConnectErrorDetailCodes', () => {
  ...
  it('uses the OpenClaw v4 gateway protocol range', () => {
    expect(GATEWAY_MIN_CLIENT_PROTOCOL_VERSION).toBe(4)
