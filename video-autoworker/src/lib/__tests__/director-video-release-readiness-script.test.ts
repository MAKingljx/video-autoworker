...
async function writeProductMarkers(productRoot: string) {
...
describe('director video release readiness verifier', () => {
  ...
  it('accepts the exact official video-command layout with unchanged director and task payloads', () => {
    ...
    expect(result.videoCommand).toMatchObject({ version: '0.5.16' })
  ...
  it('accepts omission of non-runtime video-command auxiliaries', async () => {
    ...
    expect(verifyInstalledReleasePayloads({
      ...
      profileStateRoot: profileRoot,
      workspaceRoot,
    }).videoCommand).toMatchObject({ version: '0.5.16' })
  })
...
`)
  ...
  it('binds the same source closure when the product is prefixed in the repository tree', async () => {
    ...
    const artifactRoot = join(root, 'prefixed-provenance-artifact')
    await mkdir(productRoot, { recursive: true, mode: 0o700 })
    await mkdir(artifactRoot, { recursive: true, mode: 0o700 })
    for (const member of DIRECTOR_EXTRACTION_SOURCE_ROOTS) {
      await mkdir(dirname(join(productRoot, member)), { recursive: true })
      await writeFile(join(productRoot, member), `fixture:${member}\n`)
    }
    await writeProductMarkers(productRoot)
    const git = (...args: string[]) => execFileSync('git', ['-C', gitRoot, ...args], {
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'pipe'],
    }).trim()
    git('init', '-b', 'main')
    git('config', 'user.name', 'Prefixed Provenance Test')
    git('config', 'user.email', 'prefixed-provenance@example.invalid')
    git('config', 'commit.gpgSign', 'false')
    git('add', '.')
    git('commit', '-m', 'prefixed source closure')
    const commit = git('rev-parse', 'HEAD')

    writeProvenanceOnly(artifactRoot, productRoot)
    await writeProvenanceOnlyManifest(artifactRoot)
    expect(verifyDirectorExtractionReleaseProvenance({
  ...
  it.each([
    ['d23', 'd23c015314f73b9a178f43d97d1b710e1bd36394'],
    ...
    expect(closure.files.every((entry: { path: string }) => (
      !entry.path.startsWith('video-autoworker/')
    ))).toBe(true)
  })
  ...
  it('accepts a fixed clean application commit on canonical mainline without requiring a branch checkout', async () => {
    const gitRoot = join(root, 'pinned-application')
    await mkdir(gitRoot, { mode: 0o700 })
    const git = (...args: string[]) => execFileSync('git', ['-C', gitRoot, ...args], {
      encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'],
    }).trim()
    ...
    expect(assertRepositoryRelease(gitRoot, `${commit}-runtime`, 'head')).toBe(commit)
    await writeFile(join(gitRoot, 'local-only.txt'), 'unpublished')
    expect(() => assertRepositoryRelease(gitRoot, `${commit}-runtime`, 'head')).toThrow('repository_release_mismatch')
    git('add', '.')
    git('commit', '-m', 'unpublished change')
    const unpublished = git('rev-parse', 'HEAD')
    expect(() => assertRepositoryRelease(gitRoot, `${unpublished}-runtime`, 'head')).toThrow('repository_release_mismatch')
    git('checkout', '--detach', commit)
    git('remote', 'set-url', 'origin', 'https://example.invalid/unrelated.git')
    expect(() => assertRepositoryRelease(gitRoot, `${commit}-runtime`, 'head')).toThrow('repository_remote_mismatch')
  })
