...
const DIRECTOR_BRAIN_SERVICE_SHA256 = 'ce91dd84f55a115f7aba1b56b763631dfc68bbcb0d440f6c1a383b1de332e01e'
...
export function directorCommandTimeoutMs(
  ...
  if (command === 'operate' && input.action === 'get_many' && Array.isArray(input.stableIds)) {
    return null
  }
...
export async function runDirectorCommand(
  ...
  return await new Promise((resolvePromise, reject) => {
    ...
    const timeoutMs = directorCommandTimeoutMs(command, input)
    if (timeoutMs !== null) {
      timer = setTimeout(() => fail(new Error('director_command_timeout')), timeoutMs)
    }
    try {
