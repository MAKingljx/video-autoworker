...
function getConfigRuntimeAgentId(agent: CoordinatorAgentRecord): string | null {
  const parsed = parseConfig(agent.config)
  return typeof parsed.runtimeAgentId === 'string' && parsed.runtimeAgentId.trim()
...
export function resolveCoordinatorDeliveryTarget(params: {
  ...
  const buildResult = (
    ...
    const runtimeAgentId = getConfigRuntimeAgentId(agent) || getConfigOpenClawId(agent) || normalizeOpenClawId(agent.name)
  ...
  if (normalizedTo === normalizedCoordinatorAgent) {
    ...
    if (mainSession) {
      ...
      return {
        ...
        runtimeAgentId:
          getConfigRuntimeAgentId(matchingAgent || { name: mainSession.agent }) ||
          getConfigOpenClawId(matchingAgent || { name: mainSession.agent }) ||
          normalizeOpenClawId(mainSession.agent),
  ...
  return {
    ...
    runtimeAgentId: normalizeOpenClawId(params.to),
    resolvedBy: 'fallback',
  }
}
