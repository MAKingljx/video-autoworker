...
import { runMigrations, assertDatabaseSchemaCurrent } from './migrations';
...
import { initWebhookListener } from './webhooks';
...
export function getDatabase(): Database.Database {
  if (!db) {
    const prepareOnOpen = isBuildPhase || process.env.MISSION_CONTROL_TEST_MODE === '1'
      || process.env.NODE_ENV !== 'production';
    // Production opens an existing prepared database. Starting a web/worker
    // process must not create credentials, a replacement database or schema.
    if (prepareOnOpen) {
      ensureAutoGeneratedCredentials();
      ensureDirExists(dirname(DB_PATH));
    }
    try {
      db = new Database(DB_PATH, { fileMustExist: !prepareOnOpen });
    } catch (err: any) {
    ...
    try {
      // Journal mode is persistent. Production verifies it instead of changing
      // an unprepared database merely because a web request opened it.
      if (prepareOnOpen) db.pragma('journal_mode = WAL');
      else if (db.pragma('journal_mode', { simple: true }) !== 'wal') {
        throw new Error('database_wal_prepare_required');
      }
      db.pragma('synchronous = NORMAL');
      db.pragma('cache_size = 1000');
      db.pragma('foreign_keys = ON');
      db.pragma('busy_timeout = 5000');
      if (prepareOnOpen) initializeSchema();
      else assertDatabaseSchemaCurrent(db);
      // Local mutations retain their existing webhook delivery owner. IPC is
      // only a transient UI notification relay, never a durable delivery queue.
      if (!isBuildPhase && process.env.AIWORKER_SCHEDULER_MODE !== 'worker') initWebhookListener();
    } catch (error) {
      db.close();
      db = null;
      throw error;
